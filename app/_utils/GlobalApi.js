...
const GetBusiness=async(category)=>{
  const query=gql`
  query GetBusiness {
    fournisseurs(first:50, where: {categories_some: {slug: "`+category+`"}}) {
      aboutUs
      address
      banner {
        url
...
const GetBusinessDetail=async(businessSlug)=>{
  const query = gql`
  query FournisseurDetail {
    fournisseur(where: {slug: "`+businessSlug+`"}) {
      aboutUs
      address
      banner {
        url
...
const AddToCart=async(data)=>{
  ...
  mutation AddToCart {
    createUserCart(
      ...
      fournisseur: {connect: {slug: "`+data.fournisseurSlug+`"}}}
    ) {
      id
    }
...
const DisconnectRestroFromUserCartItem=async(id)=>{
  const query=gql`
  mutation DisconnectFournisseurFormCartItem {
    updateUserCart(data: {fournisseur: {disconnect: true}}, where: {id: "`+id+`"})
    {
      id
    }
    publishManyUserCarts(to: PUBLISHED) {
...
const getRestaurantReviews=async(slug)=>{
  const query=gql`
  query FournisseurReviews {
    reviews(where: {fournisseur: {slug: "`+slug+`"}},orderBy: publishedAt_DESC) {
      email
      id
      profileImage
      publishedAt
